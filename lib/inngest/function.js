  ...
  async ({ step }) => {
    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        console.log(`[Budget Alert] lastAlertSent: ${budget.lastAlertSent}`);

        // Check if we should send an alert - lowered threshold to 50% for testing
        if (percentageUsed >= 50) {
          // Default threshold of 50% (lowered for testing)
          const shouldSend =
            !budget.lastAlertSent ||
            isNewMonth(new Date(budget.lastAlertSent), new Date());

          console.log(
            `[Budget Alert] Threshold met (${percentageUsed.toFixed(2)}%), Should send: ${shouldSend}`
          );

          if (shouldSend) {
            console.log(`[Budget Alert] Sending email to ${budget.user.email}`);
